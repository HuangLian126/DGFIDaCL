...
def do_train(model, data_loader, optimizer, scheduler, checkpointer, device, checkpoint_period, arguments, data_loader_closeup, oneStage, gamma, margin, shot):
    ...
    for iteration, (images, targets, _) in enumerate(data_loader, start_iter):
        ...
        sup_imgs, sup_target = next(data_iter_closeup)    # [15, 1, 8, 224, 224]

        sup = [sup_img.to(device) for sup_img in sup_imgs]
        ...
        if iteration % 20 == 0 or iteration == max_iter:
            logger.info(meters.delimiter.join(["eta: {eta}", "iter: {iter}","{meters}","lr: {lr:.6f}","max mem: {memory:.0f}"]).format(
                    eta=eta_string,
                    iter=iteration,
                    meters=str(meters),
