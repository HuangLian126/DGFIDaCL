...
class FastRCNNLossComputation(object):
    ...
    def __call__(self, class_logits, box_regression, xc_cpe_normalized, sup_normalized, label_sup):
        ...
        if xc_cpe_normalized is not None:
            ...
            ############################################################################################################
            fg_index = labels != 0
            xc_cpe_normalized_fg = xc_cpe_normalized[fg_index]  # [m, 128]
            labels_fg = labels[fg_index]                        # [m]

            # numel = xc_cpe_normalized_fg.shape[0]             # m is named numel
            ...
            prototype_loss = -log_prob
            prototype_loss = 0.15 * (prototype_loss.mean())
            # print('prototype_loss: ', prototype_loss)
            ############################################################################################################


            ############################################################################################################
            labels_trans_psal = labels_fg.unsqueeze(1)  # [m, 1]
            label_mask_psal = torch.eq(labels_trans_psal, labels_trans_psal.t()).float().cuda()  # [512, 512]
            logits_mask_psal = torch.ones_like(label_mask_psal)
            logits_mask_psal_diag = torch.diagonal(logits_mask_psal)
            logits_mask_psal_diag_nxn = torch.diag_embed(logits_mask_psal_diag)
            ...
            mask = logits_mask_psal * label_mask_psal
            keep = (mask.sum(1) != 0)
            log_prob_psal = torch.log((exp_sim_psal[keep] * mask[keep]).sum(1) / (exp_sim_psal[keep] * logits_mask_psal[keep]).sum(1))

            # log_prob_psal = torch.log((exp_sim_psal * mask).sum(1) / (exp_sim_psal * logits_mask_psal).sum(1))
            proposal_loss = -log_prob_psal
            proposal_loss = 0.15 * (proposal_loss.mean())
            # print('proposal_loss: ', proposal_loss)
            ############################################################################################################
            ...
            box_loss = smooth_l1_loss(box_regression[sampled_pos_inds_subset[:, None], map_inds],
                                      regression_targets[sampled_pos_inds_subset], size_average=False, beta=1)
            box_loss = box_loss / labels.numel()

            return classification_loss, box_loss, proposal_loss+prototype_loss
        else:
