...
class FPN(nn.Module):
    ...
    def forward(self, x):
        ...
        Returns:
            results (tuple[Tensor]): feature maps after FPN layers. They are ordered from highest resolution first.
        """
        ...
        '''
        x[0].shape:  torch.Size([1, 256,  48, 48])
        x[1].shape:  torch.Size([1, 512,  24, 24])
        x[2].shape:  torch.Size([1, 1024, 12, 12])
        x[3].shape:  torch.Size([1, 2048, 6,  6])
        '''

        last_inner = getattr(self, self.inner_blocks[-1])(x[-1])  # 1, 256, 6, 6
        # print('last_inner.shape1: ', last_inner.shape)
        ...
        for feature, inner_block, layer_block in zip(x[:-1][::-1], self.inner_blocks[:-1][::-1], self.layer_blocks[:-1][::-1]):
            ...
            # print('inner_top_downvvv: ', inner_top_down.shape)
