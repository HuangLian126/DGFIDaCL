...
class RPNHead(nn.Module):
    ...
    def forward(self, x, closeup=False):
        if not closeup:
            ...
            # x is the list[Tensor], Tensor: num_closeups x channel x 8 x 8
            logits = []
            for i in range(len(x)):
                for j in range(len(x[i])):
                    if j is not 4:
                        t = F.relu(self.conv(self.GAP(x[i][j])))
                    else:
                        t = F.relu(self.conv(x[i][j]))
                    cls_conv = self.cls_conv(t)
                    cls_conv = cls_conv.view(cls_conv.size(0) * self.num_anchors, self.mid_channels, cls_conv.size(2), cls_conv.size(3))
                    cls_logits = self.cls_logits(cls_conv).view(-1, self.num_anchors, cls_conv.size(2), cls_conv.size(3))
...
class RPNModule(torch.nn.Module):
    ...
    def forward(self, images, features, targets=None, supp = None):
