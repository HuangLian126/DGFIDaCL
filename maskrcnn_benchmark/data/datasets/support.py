...
from maskrcnn_benchmark.data.datasets.seed_init import place_seed_points
...
class PascalVOCDataset_Meta(torch.utils.data.Dataset):
    ...
    def __init__(self, data_dir, split, use_difficult=False, transforms=None, toofew=True, shots=200, size=224, seed=0):
        ...
        self.list_root = os.path.join('/home/hl/hl/DCNet-main/', 'fs_list')
    ...
    def __getitem__(self, index):
        ...
        for cls_id, img_id in img_ids:
            ...
            #######算超像素的######
            target_pix = np.where(support_label == 1)
        ...
        for i in range(0, shot):
            ...
            init_seed = place_seed_points(mask, down_stride=8, max_num_sp=5, avg_sp_area=50)
        ...
        s_x = s_x.permute(0, 3, 1, 2)
        s_y = s_y.unsqueeze(1)

        res2 = torch.cat((res, s_x, s_y), dim=1)

        # print('res: ', res.shape)
        # print('s_x: ', s_x.shape)
        # print('s_y: ', s_y.shape)
        # print('s_init_seed: ', s_init_seed.shape)

        # return res, s_x, s_y, s_init_seed

        return res2, s_init_seed
